...
class XlsxPropertyReport(http.Controller):
    ...
    def download_property_excel_report(self,property_ids):
        property_ids = request.env['property'].browse(literal_eval(property_ids)) #literal_eval takes sting in form of lists and returns list
        # browse returns records
        ...
        for col_num,header in enumerate(headers): #--> enumerate returns number and value
        ...
        workbook.close()
        output.seek(0) #to read workbook from begining
        ...
        return request.make_response(
            ...
            headers = [
                ...
                ('Content-Disposition', f'attachment; filename={file_name}') # to make file name dynamic
            ]
